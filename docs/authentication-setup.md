...
Для временной QA-проверки можно добавить в локальный `server/.env` `AUTH_FIXED_VERIFICATION_CODE=111111`. В production сервер всегда использует случайные коды.
